...
edl_ox_scale_list = np.array((0.5, 0.75, 1.0))
...
for muscle_name in ('SOL', 'EDL'):
    ...
    peak_qr_by_muscle[muscle_name] = np.zeros((len(edl_ox_scale_list), len(freq_list)))
    tau_by_muscle[muscle_name] = np.zeros((len(edl_ox_scale_list), len(freq_list)))

    # Deep copy the parameter value
    V_max_oxphos_val = copy.deepcopy(params[muscle]['V_max_oxphos'])

    # Loop over Q10 values 
    for idx_q10, edl_scale_oxphos in enumerate(edl_ox_scale_list): 
        # peak_qr_vs_freq = []
        # tau_vs_freq = []
        ...
        for idx, freq in enumerate(freq_list): 
            ...
            if muscle == 'EDL':
                params[muscle]['V_max_oxphos'] = V_max_oxphos_val * edl_scale_oxphos
            ...
            ax_tau.plot(t_rel, exp_decay(t_rel, *popt), '--', color = palette[idx_q10], label = f'q10 = {edl_scale_oxphos}, f = {freq} Hz fit ($\\tau$ = {tau_fit:.2f} s)')
            ...
            efficiency_rows.append((edl_scale_oxphos, freq, eta_init, eta_total, efficiency_ratio))
    ...
    print('edl_scale_oxphos | freq_Hz | eta_init = W/E_tot | eta_total = W/(E_tot + E_rec) | E_rec / E_tot')
    print('----------+---------+---------------------+-------------------------------+---------------')
    for edl_scale_oxphos, freq, eta_init, eta_total, efficiency_ratio in efficiency_rows:
        print(f'{edl_scale_oxphos:10.2f} | {freq:7.2f} | {eta_init:19.6f} | {eta_total:29.6f} | {efficiency_ratio:14.6f}')
...
for idx_q10, scale in enumerate(edl_ox_scale_list):
...
fig_peak_qr_compare.savefig('Figures/BW_edlscalesens_rrecmax.jpg')
...
fig_tau_freq_compare.savefig('Figures/BW_edlscalesens_tau.jpg')
