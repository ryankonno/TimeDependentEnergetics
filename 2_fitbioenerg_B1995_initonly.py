...
params = {
    ...
    'muscle': 'SOL', # Specify muscle parameters to be used in simulation
        ...
        'SOL': {
            ...
            # 'nh': 0.01, # unitless, V/Icini 2000, .... TBD (may need to optimise for this parameter)
            ...
            'r_rec':  43.3e3, # J / mol, Optimised value
...
# # Print out the optimal solution 
# x = opt_res.x 
# print(f'V_max_oxphos = {x[0]}')
# params[muscle]['V_max_oxphos']= x[0]

# Opt V_max, nh, K_adp
# bounds_ = ((0.5,5), (0.001,0.4), (0.1,10))

# Previous version (kept for reference): random multi-start + Nelder-Mead
# rng = np.random.default_rng()
# n_random_starts = 10
# best_fun = np.inf
# best_res = None
#
# for i_start in range(n_random_starts):
#     x0 = tuple(rng.uniform(low, high) for low, high in bounds_)
#     print(f'Random start {i_start + 1}/{n_random_starts}: nh = {x0[0]}, K_adp = {x0[1]}, V_max_oxphos = {x0[2]}')
#
#     iter = 0
#     opt_res_i = minimize(f_opt, x0, callback= callback_fun, bounds = bounds_,  options = {'maxiter': 500, 'disp': True}, method='Nelder-Mead')
#     print(f'Random start {i_start + 1}: final objective = {opt_res_i.fun}')
#
#     if opt_res_i.fun < best_fun:
#         best_fun = opt_res_i.fun
#         best_res = opt_res_i
#
# opt_res = best_res
# print(f'Best objective across {n_random_starts} starts = {best_fun}')

# # Global optimisation with differential evolution
# de_seed = 42
# opt_res_de = differential_evolution(
#     f_opt,
#     bounds_,
#     strategy='best1bin',
#     maxiter=50,
...
#     polish=False,
# )
# print(f'DE result: nh = {opt_res_de.x[0]}, K_adp = {opt_res_de.x[1]}, V_max_oxphos = {opt_res_de.x[2]}')
# print(f'DE objective = {opt_res_de.fun}')

# # Local polish from DE best point
# iter = 0
# opt_res = minimize(
#     f_opt,
#     opt_res_de.x,
...
#     method='Nelder-Mead',
# )
# print(f'Polished objective = {opt_res.fun}')
# # Print out the optimal solution 
# x = opt_res.x 
# print(f'nh = {x[0]}, K_adp = {x[1]}, V_max_oxphos = {x[2]}')
# # update the parameter values , note multiplication factors
