...
params = {
    ...
    # 'Energetics_model':{
        ...
        'SOL': {
            ...
            'r_rec': 1 / 0.76 * 60e3, # J/ s / L, Assumes mitochondrial efficiency based on average mouse sol 
...
class Bioenergetics(): 
    ...
    def phi_atp(self, t, c_atp): 
        ...
        return  self.k_rest * c_atp *  (t <= t_start_cycle)\
                 + (self.k_rest * c_atp + ( atp_peak) * (t_cycle < tstimend)) * (t > t_start_cycle) * (t <= t_end_cycle)\
                 + self.k_rest * c_atp *  (t > t_end_cycle)
    ...
    def solveBioenergetics(self, t_span, c_atp_0):
        ...
        sol = solve_ivp(self.rhs, t_span, y_0, "BDF", max_step = 0.001)
...
for muscle in ("SOL", "EDL"):
    ...
    for i, freq in enumerate(freq_list):
        ...
        print(f'    Integral of initial heat = {init_heat_int}')
        print(f'    Integral of recovery heat = {rec_heat_int}')
