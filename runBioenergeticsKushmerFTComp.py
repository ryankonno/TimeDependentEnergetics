...
Adapted to illustrate the differences between fast and slow type fibres
...
# [0.04685323 1.19819909 0.23897525]
        ...
        'r_rec': 1 / 0.8 * 60e3 # J/ s / L, Assumes mitochondrial efficiency based on average mouse sol and edl
    # }
}
...
class Bioenergetics(): 
    def __init__(self, params, muscle = 'sol'): 
...
fig_ener, axs_ener = plt.subplots(layout = 'constrained', figsize = (6, 4))
...
for muscle in ('SOL', 'EDL'):
    ...
    model = Bioenergetics(params, muscle) 
    ...
    ax = axs_ener 
    ax.plot(sol.t, model.computeRecoveryEnergetics(sol.y[0,]), label = muscle)
    ax.set_xlabel('Time (s) ')
    ax.set_ylabel('Energetic rate (J/s/kg)')
    ax.legend()
