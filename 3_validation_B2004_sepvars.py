...
freq_for_trace_plot = freq_list[3]
trace_plot_data = None
...
for idx, freq in enumerate(freq_list): 
    ...
    # fig, ax = plt.subplots(layout = 'constrained')d
    # ax.plot(t_vec, e_ce) 
    ...
    # force_damped = mech_model.solveModel(t_vec, catn_vec, e_ce + 1, dedt_ce, params[muscle]['l_0'], \
                                        ...
                                        #       l_see_rat=0.1)
    # Store one-frequency traces for a dedicated summary figure.
    if np.isclose(freq, freq_for_trace_plot):
        trace_plot_data = {
            'freq': freq,
            't_cycle': t_cycle,
            'e_ce_cycle': e_ce_cycle,
            'stim_cycle': stim_protocol_vec[cycle_mask],
    ...
    ax_energy.plot(t_vec, cumtrapz(w, t_vec, initial = 0) * energy_unit_scaler, label = '$ q_r + e_{init}$', color = 'k', alpha = 0.25) 
    # ax_energy.legend()
...
print('freq_Hz | eta_init = W/E_init | eta_total = W/(E_init + E_rec) | E_init / E_tot - 1')
...
if trace_plot_data is not None:
    fig_trace, axs_trace = plt.subplots(1, 2, layout='constrained', figsize=(12, 4.5))

    axs_trace[0].plot(trace_plot_data['t_cycle'], trace_plot_data['e_ce_cycle'], label='$e_{ce}$', color='#1f77b4')
    stim_mask_cycle = trace_plot_data['stim_cycle'] > 0
    if np.any(stim_mask_cycle):
        t_stim_cycle = trace_plot_data['t_cycle'][stim_mask_cycle]
        axs_trace[0].axvspan(t_stim_cycle[0], t_stim_cycle[-1], color='#f94144', alpha=0.2, label='Stimulation')
    axs_trace[0].set_xlabel('Time within cycle (s)')
    axs_trace[0].set_ylabel('Strain')
    axs_trace[0].grid(True, alpha=0.3)
    axs_trace[0].legend()

    axs_trace[1].plot(trace_plot_data['t_cycle'], trace_plot_data['force_cycle'], color='#2ca02c', label='Force')
    axs_trace[1].set_xlabel('Time within cycle (s)')
    axs_trace[1].set_ylabel('Force (normalised)')
    axs_trace[1].grid(True, alpha=0.3)
    axs_trace[1].legend()

    # fig_trace.savefig('Figures/B2004_SepVars_MechConditions.jpg')
    # fig_trace.savefig('Figures/B2004_SepVars_MechConditions.svg')
...
ax_force_cycle.set_xlabel('Normalised cycle time ($t/T_{cycle}$)')
ax_force_cycle.set_ylabel('Force (normalised)')
ax_force_cycle.grid(True, alpha=0.3)
ax_force_cycle.legend()
...
# ax_comp_rel.set_ylim((0, 1))
ax_comp_rel.legend()
# fig_comp_rel.savefig('Figures/B2004_RelContr_' + params['muscle'] + '.jpg')
# fig_comp_rel.savefig('Figures/B2004_RelContr_' + params['muscle'] + '.svg')

# Plot peak recovery rate versus frequency
fig_peak_qr, ax_peak_qr = plt.subplots(layout = 'constrained')
ax_peak_qr.plot(freq_list, peak_qr_vs_freq, '-o', color = component_colors[-1])
ax_peak_qr.set_xlabel('Cycle frequency (Hz)')
ax_peak_qr.set_ylabel('Peak recovery rate ($mW g^{-1}$)')
ax_peak_qr.grid(True, alpha = 0.3)
