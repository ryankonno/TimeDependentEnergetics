...
params = {
    ...
    'muscle': 'EDL', # Specify muscle parameters to be used in simulation
        ...
        'SOL': {
            ...
            # 'r_am': 0.354, # W/F_0/l_0, Maximum heat rate of isometric contraction (slow-type fibre)
            # 'r_sl': 0.234, # W/F_0/l_0, Maximum shortening heat rate (slow-type fibre)
            'r_am': 0.5 * 0.290, # W/F_0/l_0, Maximum heat rate of isometric contraction (slow-type fibre)
            'r_sl': 0.5 * 0.238, # W/F_0/l_0, Maximum shortening heat rate (slow-type fibre)
        ...
        'EDL': { 
            ...
            # Energetics model 
            # Values for human time constants
            # 'r_am': 1.150 , # W/F_0/l_0, Maximum heat rate of isometric contraction (fast-type fibre)
            # 'r_sl': 0.704, # W/F_0/l_0, Maximum shortening heat rate (fast-type fibre)
            'r_am': 0.5 * 0.822 , # W/F_0/l_0, Maximum heat rate of isometric contraction (fast-type fibre)
            'r_sl': 0.5 * 1.015, # W/F_0/l_0, Maximum shortening heat rate (fast-type fibre)
...
def f_stim_length(t, params): 
    ...
    t_short_start = 0.06 * cycle_length
    t_length_start = 0.2 * cycle_length
...
freq_list = (0.5, 1, 2, 3, 4, 5, 6, 7, 8) # Hz, Frequencies for the cycles 
...
for muscle in ('SOL', 'EDL'):
    ...
    print(f'\n=== Running muscle: {muscle} ===')

    # INitialise plots for the current muscle
    ...
    for idx, freq in enumerate(freq_list): 
        ...
        q_r = bioenergetic_model.computeRecoveryEnergetics(sol.t, sol.y[0,]) * scale # F0l0/s
        ...
        t_cycle_end = params['cycle_length'] * params['N_cycles']
        t_peak_end = min(t_cycle_end + 50.0, sol.t[-1])
        peak_window_mask = (sol.t >= t_cycle_end) * (sol.t <= t_peak_end)
        if np.any(peak_window_mask):
            peak_qr_vs_freq.append(np.max((q_r * energy_unit_scaler)[peak_window_mask]))
        else:
            peak_qr_vs_freq.append(np.nan)
        ...
        efficiency_rows.append((freq, eta_init, eta_total))
        ...
        print(f'{muscle} {freq} Hz: fitted time constant (tau) = {tau_fit:.3f} s')

    ax_tau.set_title(f'{muscle} - Recovery Time Constant')
    ax_tau.set_xlabel('Time since end of stimulation (s)')
    ax_tau.set_ylabel('Total energy rate ($mW g^{-1}$)')
    ax_tau.legend()

    axs_strain_cycle[0].set_title(f'{muscle} - One-Cycle Strain')
    ...
    ax_comp_abs.set_title(f'{muscle} - Absolute Contributions')
    ...
    ax_comp_rel.set_title(f'{muscle} - Relative Contributions')
    ...
    peak_qr_by_muscle[muscle] = np.array(peak_qr_vs_freq)
    tau_by_muscle[muscle] = np.array(tau_vs_freq)

    # Print efficiency table in terminal for the current muscle.
    print(f'\n{muscle} efficiency table')
    print('freq_Hz | eta_init = W/E_tot | eta_total = W/(E_tot + E_rec)')
    print('--------+---------------------+-------------------------------')
    for freq, eta_init, eta_total in efficiency_rows:
        print(f'{freq:7.2f} | {eta_init:19.6f} | {eta_total:29.6f}')
