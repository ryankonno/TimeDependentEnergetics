...
for muscle_type in ['SOL', 'EDL']:
    ...
    t_vec = np.linspace(params['t_start'], params['t_end'], int(2000 * params['t_end']))
    ...
    # Plot a comparison of the energetic rates for both the cross-bridge and activation heats 
    fig, ax = plt.subplots(layout = 'constrained',figsize = (7,5)) 
    ax.plot(t_vec, q_a_int, label = 'Act-Model', color = 'k')
    # ax.plot(t_vec, data_exp_act_interp, label = 'Act-Experiment', ls = ':', color = 'k')
    ax.plot(t_vec, q_m_int, label = 'Cxb-Model', color = 'r')
    # ax.plot(t_vec, data_exp_cxb_interp, label = 'Cxb-Experiment', ls = ':', color = 'k')
    ax.axhline(data_exp_act, color='k', ls='--', lw=1.2, label='Act-Experiment')
    ax.axhline(data_exp_cxb, color='r', ls='--', lw=1.2, label='Cxb-Experiment')
    ax.set_xlabel('Time ($s$)')
    ax.set_ylabel('Heat ($F_0 l_0$)')
    ax.set_title(muscle)
    ax.legend()
